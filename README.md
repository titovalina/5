...
### Алгоритм решения
1. Создать интерфейс Fractionable с методами:
* getValue() — возвращает вещественное значение дроби.
* setNumerator(int) и setDenominator(int) — изменение числителя и знаменателя.
2. Создать класс Fraction с полями numerator и denominator.
3. В конструкторе проверить знаменатель:
* Если 0, выбросить ошибку.
* Если отрицательный, поменять знак у числителя и знаменателя.
4. Реализовать метод getValue():
* Если кэш пустой, вычислить numerator / denominator и сохранить в кэш.
* Если кэш есть, вернуть его.
5. В методах setNumerator и setDenominator инвалидировать кэш.
6. Переопределить equals() и hashCode() для сравнения дробей по значению (через cross-multiply или сокращённую дробь).
...
public void meow();
<img width="576" height="176" alt="image" src="https://github.com/user-attachments/assets/89de7ab3-d877-4194-9d88-cfde74003c8f" />


### Алгоритм решения
1. Создаём класс Cat с именем и методом преобразования в строку.
2. Создаём интерфейс Meowable с методом meow().
3. Создаём класс-декоратор MeowCounter, который:
* принимает объект типа Meowable;
* считает количество вызовов метода meow;
* передаёт вызов оригинальному объекту.
4. Создаём утилитарный класс MeowUtils, который вызывает мяуканье у всех переданных объектов.
5. В main передаём кота в метод, после выполнения выводим количество мяуканий.
...
### Алгоритм решения
1. Принимаем массив точек.
2. Преобразуем отрицательные координаты Y в положительные.
3. Убираем дублирующиеся точки.
4. Сортируем точки по координате X.
5. Собираем точки в объект Polyline.
6. Выводим ломаную линию.
